...
#include "R3F_LOG_ENABLE.h"
	...
	call compileScript ["R3F_LOG\en_strings_lang.sqf"];
	...
	if (isServer) then
	{
		...
		[] spawn compileScript ["R3F_LOG\surveiller_objets_a_proteger.sqf"];
	};
	...
	call compileScript ["R3F_LOG\fonctions_generales\lib_geometrie_3D.sqf"];
	...
	R3F_LOG_FNCT_determiner_fonctionnalites_logistique = compileScript ["R3F_LOG\fonctions_generales\determiner_fonctionnalites_logistique.sqf"];

	R3F_LOG_FNCT_calculer_chargement_vehicule = compileScript ["R3F_LOG\transporteur\calculer_chargement_vehicule.sqf"];
	R3F_LOG_FNCT_transporteur_charger_auto = compileScript ["R3F_LOG\transporteur\charger_auto.sqf"];
	...
	if !(isDedicated) then
	{
		...
		call compileScript ["R3F_LOG\fonctions_generales\lib_visualisation_objet.sqf"];

		R3F_LOG_FNCT_objet_relacher = compileScript ["R3F_LOG\objet_deplacable\relacher.sqf"];
		R3F_LOG_FNCT_objet_deplacer = compileScript ["R3F_LOG\objet_deplacable\deplacer.sqf"];

		R3F_LOG_FNCT_heliporteur_heliporter = compileScript ["R3F_LOG\heliporteur\heliporter.sqf"];
		R3F_LOG_FNCT_heliporteur_larguer = compileScript ["R3F_LOG\heliporteur\larguer.sqf"];
		R3F_LOG_FNCT_heliporteur_init = compileScript ["R3F_LOG\heliporteur\heliporteur_init.sqf"];

		R3F_LOG_FNCT_remorqueur_detacher = compileScript ["R3F_LOG\remorqueur\detacher.sqf"];
		R3F_LOG_FNCT_remorqueur_remorquer_deplace = compileScript ["R3F_LOG\remorqueur\remorquer_deplace.sqf"];
		R3F_LOG_FNCT_remorqueur_remorquer_direct = compileScript ["R3F_LOG\remorqueur\remorquer_direct.sqf"];
		R3F_LOG_FNCT_remorqueur_init = compileScript ["R3F_LOG\remorqueur\remorqueur_init.sqf"];

		R3F_LOG_FNCT_transporteur_charger_deplace = compileScript ["R3F_LOG\transporteur\charger_deplace.sqf"];
		R3F_LOG_FNCT_transporteur_charger_selection = compileScript ["R3F_LOG\transporteur\charger_selection.sqf"];
		R3F_LOG_FNCT_transporteur_decharger = compileScript ["R3F_LOG\transporteur\decharger.sqf"];
		R3F_LOG_FNCT_transporteur_selectionner_objet = compileScript ["R3F_LOG\transporteur\selectionner_objet.sqf"];
		R3F_LOG_FNCT_transporteur_voir_contenu_vehicule = compileScript ["R3F_LOG\transporteur\voir_contenu_vehicule.sqf"];
		R3F_LOG_FNCT_transporteur_init = compileScript ["R3F_LOG\transporteur\transporteur_init.sqf"];

		R3F_LOG_FNCT_usine_remplir_liste_objets = compileScript ["R3F_LOG\usine_creation\remplir_liste_objets.sqf"];
		R3F_LOG_FNCT_usine_creer_objet = compileScript ["R3F_LOG\usine_creation\creer_objet.sqf"];
		R3F_LOG_FNCT_usine_ouvrir_usine = compileScript ["R3F_LOG\usine_creation\ouvrir_usine.sqf"];
		R3F_LOG_FNCT_usine_init = compileScript ["R3F_LOG\usine_creation\usine_init.sqf"];
		R3F_LOG_FNCT_usine_revendre_deplace = compileScript ["R3F_LOG\usine_creation\revendre_deplace.sqf"];
		R3F_LOG_FNCT_usine_revendre_selection = compileScript ["R3F_LOG\usine_creation\revendre_selection.sqf"];
		R3F_LOG_FNCT_usine_revendre_direct = compileScript ["R3F_LOG\usine_creation\revendre_direct.sqf"];
		R3F_LOG_FNCT_recuperer_liste_cfgVehicles_par_categories = compileScript ["R3F_LOG\usine_creation\recuperer_liste_cfgVehicles_par_categories.sqf"];
		R3F_LOG_FNCT_determiner_cout_creation = compileScript ["R3F_LOG\usine_creation\determiner_cout_creation.sqf"];

		R3F_LOG_FNCT_objet_init = compileScript ["R3F_LOG\objet_commun\objet_init.sqf"];
		R3F_LOG_FNCT_objet_est_verrouille = compileScript ["R3F_LOG\objet_commun\objet_est_verrouille.sqf"];
		R3F_LOG_FNCT_deverrouiller_objet = compileScript ["R3F_LOG\objet_commun\deverrouiller_objet.sqf"];
		R3F_LOG_FNCT_definir_proprietaire_verrou = compileScript ["R3F_LOG\objet_commun\definir_proprietaire_verrou.sqf"];

		R3F_LOG_FNCT_formater_fonctionnalites_logistique = compileScript ["R3F_LOG\fonctions_generales\formater_fonctionnalites_logistique.sqf"];
		R3F_LOG_FNCT_formater_nombre_entier_milliers = compileScript ["R3F_LOG\fonctions_generales\formater_nombre_entier_milliers.sqf"];
		...
		[] spawn compileScript ["R3F_LOG\surveiller_conditions_actions_menu.sqf"];

		/* Auto-d�tection permanente des objets sur le jeu */
		[] spawn compileScript ["R3F_LOG\surveiller_nouveaux_objets.sqf"];
		 ...
		 */
		[] spawn compileScript ["R3F_LOG\systeme_protection_blessures.sqf"];
	};
