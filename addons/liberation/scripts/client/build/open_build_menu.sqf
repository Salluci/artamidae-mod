    ...
    if (_oldbuildtype != buildtype || synchro_done) then {
        ...
        lbClear 110;
        {
            ...
            if (buildtype != 8) then {
                ...
                switch (true) do {
                    case (_classnamevar == FOB_box_typename): {_entrytext = localize "STR_FOBBOX";};
                    case (_classnamevar == Arsenal_typename): {if (KP_liberation_mobilearsenal) then {_entrytext = localize "STR_ARSENAL_BOX";};};
                    case (_classnamevar == Respawn_truck_typename): {if (KP_liberation_mobilerespawn) then {_entrytext = localize "STR_RESPAWN_TRUCK";};};
                    case (_classnamevar == FOB_truck_typename): {_entrytext = localize "STR_FOBTRUCK";};
                    case (_classnamevar == "Flag_White_F"): {_entrytext = localize "STR_INDIV_FLAG";};
                    case (_classnamevar == KP_liberation_small_storage_building): {_entrytext = localize "STR_SMALL_STORAGE";};
                    case (_classnamevar == KP_liberation_large_storage_building): {_entrytext = localize "STR_LARGE_STORAGE";};
                    case (_classnamevar in KP_liberation_recycle_building): {_entrytext = localize "STR_RECYCLE_BUILDING";};
                    case (_classnamevar in KP_liberation_air_vehicle_building): {_entrytext = localize "STR_HELI_BUILDING";};
                    case (_classnamevar in KP_liberation_heli_slot_building): {_entrytext = localize "STR_HELI_SLOT";};
                    case (_classnamevar in KP_liberation_plane_slot_building): {_entrytext = localize "STR_PLANE_SLOT";};
                    default {_entrytext = getText (_cfg >> _classnamevar >> "displayName");};
                };
