...
while {true} do {
    waitUntil {
      sleep 0.2;
      (GRLIB_force_redeploy) || ((player distance (markerPos GRLIB_respawn_marker) < 50) && vehicle player == player && alive player && !dialog)
    };
    ...
    if (dialog && deploy == 1) then {
        ...
        if ((lbCurSel 203) > 0) then {
            ...
            if (KP_liberation_ace && KP_liberation_arsenal_type) exitWith {
    ...
    if (dialog) then {
        closeDialog 0;
        if (isNil "APM_HintDone" && (player getVariable "APM_Rank") < 3) exitWith
        {
          APM_HintDone = true;
