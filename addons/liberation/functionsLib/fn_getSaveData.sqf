...
private ["_fobPos", "_fobObjects", "_grpUnits", "_fobMines"];
{
    ...
    _fobObjects = (_fobPos nearObjects (GRLIB_fob_range * 1.2)) select {
        ...
        {!(_x getVariable ["APM_save_object",false])} &&            // Exclude objects handled by the APM system
...
private ["_savedPos", "_savedVecDir", "_savedVecUp", "_class", "_hasCrew", "_damage", "_fuel"];
{
    // Position data
    ...
    _class = typeOf _x;
    _hasCrew = false;
    _damage = [damage _x, getAllHitPointsDamage _x];
  	_fuel = fuel _x;
    ...
    if (
        ...
        _objectsToSave pushBack [_class, _savedPos, _savedVecDir, _savedVecUp, _hasCrew, _damage, _fuel];
    };
} forEach _allObjects;
